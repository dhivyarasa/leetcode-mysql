...
-- Solution to Calculate Salaries
-- Using cte
...
SELECT 
    ...
    s.employee_id,
    s.employee_name,
    ROUND(s.salary * (1 - c.tax_rate)) AS salary
FROM Salaries s
JOIN Company_Tax_Rate c ON s.company_id = c.company_id;
...
SELECT 
    ...
    ROUND(
        CASE
            WHEN max_salary < 1000 THEN s.salary
            WHEN max_salary BETWEEN 1000 AND 10000 THEN s.salary * (1 - 0.24)
...
JOIN (
    ...
    FROM Salaries
    GROUP BY company_id
) AS c ON s.company_id = c.company_id;