...
Round ctr to 2 decimal points. Order the result table by ctr 
...
-- Solution to Ads Performance
SELECT 
    ad_id,
    ROUND(IFNULL(SUM(action = 'Clicked') / NULLIF(SUM(action IN ('Clicked' , 'Viewed')), 0),
                    0) * 100,
            2) AS ctr
FROM
    Ads
